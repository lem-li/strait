...
<table cellpadding=0 cellspacing=0 class="table_form" width="100%">
        ...
        <td>
        <select name="website">
        <?php if(is_array($website)): $i = 0; $__LIST__ = $website;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><option value="<?php echo ($row['id']); ?>" <?php if(($vo['website']) == $row['id']): ?>selected="selected"<?php endif; ?>><?php echo ($row['name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
        </td>
        </tr>
        <?php else: ?>
        <input type="hidden" name="website" value="<?php echo ($userWebsite); ?>"><?php endif; ?>
	...
	<!--tr>
		  <td><?php echo L(small);?></td>
		  <td>
		  
		  <div class="small_box"  ><div id="small_aid_box" style="display:none;"></div>

		  <input type="text"  id="small" name="small"  value="<?php echo ($vo['small']); ?>" class="input-text" size="40"/><input type="button" class="button" value="<?php echo L(upload);?>" onclick="javascript:swfupload('pic_uploadfile','small','<?php echo L(uploadfiles);?>',1,1,0,10,'jpeg,jpg,png,gif',5,230,'<?php echo ($yourphp_auth); ?>',yesdo,nodo)" /> <a href="javascript:void(0);" onclick="showpicbox($('#small').val());"> <?php echo L(view);?></a></div>
		 </td>
	</tr-->
	...
	<tr>
		  ...
		  <td><textarea id="description" name="description" rows="5" cols="50"><?php echo ($vo["description"]); ?></textarea></td>
	</tr>
	<!--tr>
		  <td><?php echo L(data);?></td>
		  <td><?php echo L(slide_data);?>:top=10&left=8&time=1<br><textarea id="data" name="data" rows="10" cols="70"><?php echo ($vo["data"]); ?></textarea></td>
	</tr-->
	<tr>
