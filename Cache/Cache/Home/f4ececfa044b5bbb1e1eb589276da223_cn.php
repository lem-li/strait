...
<div class="wrap" style="overflow:hidden;">
    ...
    <div class="list_r">
        <div class="cm">
            <?php echo L(catpos);?><a href="<?php echo URL();?>"> <?php echo L(home_font);?></a> >> <?php  $arrparentid = array_filter(explode(',', $Categorys[$catid]['arrparentid'].','.$catid));foreach($arrparentid as $cid):$parsestr[] = '<a href="'.$Categorys[$cid]['url'].'">'.$Categorys[$cid]['catname'].'</a>'; endforeach;echo implode(" &gt; ",$parsestr);?> <?php if($currentyear) : ?>>> <?php echo ($currentyear); ?>年<?php endif;?>
        </div>
        <ul class="news_list">
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$r): $mod = ($i % 2 );++$i;?><li><a href="<?php echo ($r["url"]); ?>" target="_blank">> <?php echo (str_cut($r["title"],90)); ?></a><?php if($catid==30) : ?><span class="date">[<?php echo (todate($r["createtime"],'Y-m-d')); ?>]</span><?php endif;?></li><?php endforeach; endif; else: echo "" ;endif; ?> 
