...
<div class="wrap" style="overflow:hidden;">
    <div class="list_r" style="float:none;width:1000px;">
        <div class="cm">
            <?php echo L(catpos);?><a href="<?php echo URL();?>"> <?php echo L(home_font);?></a> >> <?php  $arrparentid = array_filter(explode(',', $Categorys[$catid]['arrparentid'].','.$catid));foreach($arrparentid as $cid):$parsestr[] = '<a href="'.$Categorys[$cid]['url'].'">'.$Categorys[$cid]['catname'].'</a>'; endforeach;echo implode(" &gt ",$parsestr);?> <?php echo L(SEARCH_RESULTS);?>
        </div>
        <div style="padding-top: 20px; height: 30px; color: rgb(102, 102, 102);">您输入的检索词：<font style="color:red;"><?php echo ($keyword); ?></font>，共查到<font style="color:red;"><?php echo ($count); ?></font>篇相关网页
        </div>
        <ul class="news_list">
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$r): $mod = ($i % 2 );++$i;?><li><!--<span class="date">[<?php echo (todate($r["createtime"],'Y-m-d')); ?>]</span>-->
					<?php if($r['url'] == 1) : ?>
					<a href="<?php echo U('Page/index?id='.$r['id'].'&l='.$l);?>" target="_blank"><?php echo ($r["title"]); ?></a>
					<?php else :?>
