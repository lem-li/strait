...
class Form extends Think {
	...
	public function catid($info,$value){
		...
		foreach ($Category as $r){
				...
				if( ($this->isadmin && /*$_SESSION['groupid']!=1*/ !in_array($_SESSION['groupid'], array(1,2)) && !in_array($_SESSION['groupid'],$postgroup)) ||  
                                        (empty($this->isadmin) && !in_array( cookie('groupid'),$postgroup)) ) continue;
	...
	public function editor($info,$value){
		...
		if($info['setup']['edittype']=='ckeditor'){
			...
			$str .= "<script type=\"text/javascript\">\r\n";
                        $str .= "KindEditor.options.filterMode = false;";
