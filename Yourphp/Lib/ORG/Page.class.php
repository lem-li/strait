...
class Page extends Think {
	...
	public function  show(){
		...
		if(is_array($urlrule) && isset($urlrule[1])){
			$urlrule[1] .= isset($_GET['year']) && is_numeric($_GET['year']) ? '&year='.$_GET['year'] : '';
		}
		if(!$urlrule){		
			$p = C('VAR_PAGE');			
		...
		$output .='<a href="'.$this->pageurl($urlrule,$this->totalPages,$this->parameter).'">'.L('Last_page')."</a>";
                $output .='<input type="text" value="'.$this->nowPage.'" class="page_go_mz_val" style="line-height: 22px;margin:-4px 10px 0 10px;width:30px">';
                $output .='<a class="page_go_mz_but">确定</a>';
		return $output;
	}
