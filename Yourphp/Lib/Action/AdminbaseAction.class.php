...
class AdminbaseAction extends Action
{
	...
	function _initialize()
	{
		...
		$this->mod = F('Mod');
               ...
               //var_dump($this->Config); 
                ...
                $this->website=D('Domain')->where('status=1 and type_id=0')->select();
                $website_menu = '';
                $this->websql = '';
                $this->isRoot = false;
                if(!empty($userInfo)){
                    ...
                    if($userInfo['adminid'] == 1 || $userInfo['groupid'] == 1){
                        $this->isRoot = true;
                        if(!empty($websiteid)){
                            $this->websql = $websiteid;
                        }
                        $website_menu .= '<select name="websiteid"><option value="">分站</option>';
                        foreach($this->website as $w){
                            $website_menu .= '<option value="'.$w['id'].'" '.(intval($_GET['websiteid'])==$w['id']?'selected="selected"':'').'>'.$w['name'].'</option>';
                        }
                        $website_menu .= '</select>';
                    }else{
                        $this->websql = $userInfo['website'];
                    }
                }
                $this->website_menu = $website_menu;
                $this->assign('website', $this->website);
                $this->userInfo = $userInfo;
                $this->assign('userWebsite', $userInfo['website']);
                /**
                 * END website
                 */
			...
			if($this->module[$this->moduleid]['type']==1 && ACTION_NAME=='index'){
				...
				if($this->categorys){
					foreach ($this->categorys as $r){
						...
						if(!in_array($_SESSION['groupid'],array(1,2)) /*$_SESSION['groupid']!=1*/ && !in_array($_SESSION['groupid'],explode(',',$r['postgroup']))) continue;
	...
	function _list($modelname, $map = '', $sortBy = '', $asc = false ,$listRows = 15) {
		...
		if($typeid) $map['typeid']=$typeid;
                if($type_id>-1) $map['type_id']=$type_id;
                if($this->websql){
                    $map['website']=$this->websql;
                }
	...
	public function create_index($sitemap)
    {
		...
		if(empty($sitemap)){
			...
			$this->assign('bcid',0);
			$r=$this->buildHeadPageHtml('index','./'.$lang,'./Yourphp/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Index_index'.C('TMPL_TEMPLATE_SUFFIX'));
		}else{
			$this->assign('sitemap','1');
			$r=$this->buildHeadPageHtml('sitemap','./'.$lang,'./Yourphp/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Sitemap'.C('TMPL_TEMPLATE_SUFFIX'));
		}
		if($r) return true;
    }
	...
	function getPhoto($englishMainAddress){
		$result = '';
		if(empty($englishMainAddress)){
			return '';
		}

		$photo_url = "http://192.168.1.213/arbpic/".$englishMainAddress;
		$res = file_get_contents($photo_url);
		if(isset($res) && !empty($res)){
			...
			$pathinfo = pathinfo($englishMainAddress);
			$extension = $pathinfo['extension'];
			$photonewfile = $uploadpath.'/'.uniqid().'.'.$extension;
			$rs = file_put_contents($photonewfile, $res);
			if($rs){
				$result = substr($photonewfile, 1);
			}else{
				$result = '';
			}
		}else{
