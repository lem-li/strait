...
class CategoryAction extends AdminbaseAction
{
    ...
    public function index()
    {
        if($this->categorys){
			...
			foreach($this->categorys as $r) {
				...
				if($r['module']=='Page'){
                                        $weburl = empty($webid)?'':'&websiteid='.$webid;
					$r['str_manage'] = '<a href="?g=Admin&m=Page&a=edit&id='.$r['id'].$weburl.'">'.L('edit_page').'</a> | ';
				}elseif($r['module']==''){
					$r['str_manage'] = '';
				}else{
	...
	public function delete() {
		...
		$module = $this->categorys[$catid]['module'];
                if($_GET['lll'] ==123){
                    //var_dump($this->categorys[$catid]['type']);exit;
                }
		if($this->categorys[$catid]['type']==1){
			...
			$arrchildid = $this->categorys[$catid]['arrchildid'];
                        //table _page 中无catid字段，改为 id 2015-07-11 20:15
                        if($module == 'Page'){
                            $where =  "id in(".$arrchildid.")";
                            //$this->dao->delete($catid);
                        }else{
                        ...
                        if($arrchildid){
                            ...
                            if($_GET['lll'] ==123){
                                //var_dump($count);var_dump($module);
                            }
                            if($count && $this->categorys[$catid]['module'] != 'Page') $this->error(L('category_does_not_allow_delete'));
                            $this->dao->delete($arrchildid);
                            //类型错误 moduleid错误 2015-07-11 20:15
