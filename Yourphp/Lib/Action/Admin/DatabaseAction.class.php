...
class DatabaseAction extends AdminbaseAction{
	...
	public function dumptablesql($table, $startfrom = 0, $currsize = 0,$auto_increment=0) {
        ...
        public function copyTable(){
            $webid = isset($_GET['wid']) ? $_GET['wid'] : 2;
            $list = M('Page')->where('website = 1')->select();
            //M('Page')->delete(array('where'=>'website = 2'));
            //var_dump($list);exit;
            if($list)foreach($list as $v){
//                $sql = "INSERT INTO `mz_page` values (null,'{$v['catid']}','{$v['catid']}','{$v['catid']}','{$v['catid']}','{$v['catid']}'
//                    ,'{$v['catid']}','{$v['catid']}','{$v['catid']}','{$v['catid']}','{$v['catid']}','{$webid}')";
                //var_Dump($v);//exit;
                unset($v['id']);
                $v['website'] = $webid;
                $one = M('Page')->where("catid = '{$v['catid']}' AND website = {$webid} ")->select();
                if(empty($one)){
                    M('Page')->add($v);
                }
            }
        ...
        public function copyA(){
            $webid = isset($_GET['wid']) ? $_GET['wid'] : 2;
            $list = M('Article')->where('website = 1')->select();
            if($list)foreach($list as $v){
                unset($v['id']);
                $v['website'] = $webid;
                //$one = M('Article')->where("catid = '{$v['catid']}' AND website = {$webid} ")->select();
                //if(empty($one)){
                    M('Article')->add($v);
                //}
            }
