...
class ContentAction extends AdminbaseAction
{
	...
	public function edit()
    {
		...
		if(MODULE_NAME=='Page'){
                ...
                if(MODULE_NAME=='Page'){
                    ...
                    $vo = $this->dao->find(array('where'=>"website = '{$webid}' AND catid = '{$id}'"));
                }else{
                    $vo = $this->dao->getById ( $id );
                }
                if(empty($vo) && MODULE_NAME=='Page'){
                    $vo['catid'] = $id;
                }
		$vo['content'] = htmlspecialchars($vo['content']);
 		$form=new Form($vo);
                
                $view_id = MODULE_NAME=='Page' ? $vo['catid'] : $id;
    ...
    public function insert($module='',$fields=array(),$userid=0,$username='',$groupid=0)
    {
		...
		$_POST['createtime'] = empty($_POST['createtime']) ? time() : $_POST['createtime'];		 
		$_POST['updatetime'] = $_POST['createtime'];               // var_dump(date('Y-m-d H:i:s',$_POST['createtime']));exit;
		...
		if(GROUP_NAME=='User')$_POST['status'] = $this->Role[$groupid]['allowpostverify'] ? 1 : 0;
                ...
                if(isset($_POST['rid']) && is_numeric($_POST['rid']) && $_POST['rid'] > 0){
                    $model->find($_POST['rid']);
                    $model->rid = $_POST['id'];
                    $model->save ();
                }
	...
	{  
		...
		//$olddata = $model->find($_POST['id']);
                if(MODULE_NAME == 'Page'){
                    $olddata = $model->find(array('where'=>"website = '{$_POST['website']}' AND catid = '{$_POST['id']}'"));
                }else{
                    $olddata = $model->find($_POST['id']);
                }
		if (false === $model->create ()) {
		...
		//$list=$model->save ();
                if(MODULE_NAME == 'Page'){
                    ...
                    if(empty($olddata)){
                        $sql = "insert into mz_page VALUES  (null,'{$_POST['id']}','{$title}','{$_POST['thumb']}','{$keywords}',
                                                        '{$description}','{$content}','',0,'{$_POST['lang']}',1,0,'{$_POST['website']}','')";
                        $list = $this->dao->execute($sql);                               
                    }else{
                        $sql  = "update mz_page set thumb = '{$_POST['thumb']}',title='{$title}',keywords='{$keywords}',
                                            description='{$description}',content='{$content}',lang='{$_POST['lang']}',
                                            url='{$_POST['url']}' where website = '{$_POST['website']}' AND catid = '{$_POST['id']}'";
                        $list = $this->dao->execute($sql);
                    }
                }else{
                    $list=$model->save ();
	...
	public function status(){
        ...
        public function getOnePage(){
            ...
            if(empty($webid) || !is_numeric($webid) || empty($catid) || !is_numeric($catid)){
                return ;
            }
            $datail = M('Page')->find(array('where'=>"website = '".$webid."' AND catid = '{$catid}'"));
