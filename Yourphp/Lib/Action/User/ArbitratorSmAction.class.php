...
class ArbitratorSmAction extends BaseAction {
	...
	function _initialize()
	{
		...
		$this->counturl = "http://192.168.1.213/system/arbNameMobileCount.action";  //获取列表数量
		$this->listurl = "http://192.168.1.213/system/arbNameMobileCount.action"; //获取列表
	...
	$counturl = $this->counturl;
		...
		import ( '@.ORG.Page' );
		if(LANG_NAME == 'cn'){
			$searchName = get_safe_replace($_GET['arbname']);
			$searchType = get_safe_replace($_GET['mobile']);
		...
		$list = array();
		if($countarr && isset($countarr['arbitrator'])){
			$list = $countarr['arbitrator'];
		}
 		$count = 0;
 		if($countarr && isset($countarr['caseNo'])){
 			$count = $countarr['caseNo'];
 		}
		$page=new Page($count,15);
		$show=$page->show();
		$this->assign("page",$show);
		$listgeturl = $listurl.$where."&pageBean.pageline=100&pageBean.cpage=$page->nowPage";
		...
		//print_r($list[0]['caseNo']);
		$list1 = json_decode($listjson,1);
		//测试数据
		...
		//判断用户名、手机号、验证码是否准确
		if (!empty($searchName) || !empty($searchType) || !empty($searchLanguage) || !empty($searchAddress) || !empty($searchSpecSkill))
		{
		//echo "AAAAA";
		//echo $list[0]['arbitratorId'];
		if($list[0]['caseNo']>=1 && $searchSpecSkill==$_SESSION['mobile_code'] )
		//if($list[0]['caseNo']>=1 )
		{
			//$_SESSION['searchName'] = $searchName;
			$_SESSION['searchName'] = $list[0]['arbitratorName'];
			$_SESSION['userId'] = $list[0]['arbitratorId'];
			$_SESSION['searchType'] = $searchType;
			header("Location:http://www.cietac.org/index.php?g=User&m=ArbitratorQuery&a=query"); 
		}
		else
		{ 
		print_r("请输入您正确的姓名、手机号和验证码，谢谢配合！"); 
		}
		}
		//获取国籍
		if(LANG_NAME == 'cn'){
