...
// 			$this->success(L('login_ok'));exit;
			redirect(U('User/Index/profile?l='.LANG_NAME));exit;
		}
	...
	function getpassword(){
		$username = get_safe_replace($_POST['username']);
		$useranswer = $this->dao->field("id,username,question")->where("username = '{$username}'")->find();
	...
	function checkanswer(){
		...
		$user = M('User');
		//判断邮箱是用户是否正确
		$data =$user->where("id={$userid} and username='{$username}' and answer='{$answer}'")->find();
		if($data){
			redirect(U('User/Login/repassword?userid='.$userid.'&username='.$username.'&answer='.$answer.'&l='.LANG_NAME));
		}else{
			 $this->error(L('ANSWER_ERROR'));
		}
	}
	...
	function repassword(){
		...
		$userid = get_safe_replace($_GET['userid']);
		$username = get_safe_replace($_GET['username']);
		$answer = get_safe_replace($_GET['answer']);
    ...
    function noagreement()
    {
    	if($this->_userid) {
    		...
    		//$this->assign('jumpUrl',$this->forward);
    		//$this->success(L('loginouted'));
    		redirect(U('User/Login/index?l='.LANG_NAME));
    	}else {
    		//$this->assign('jumpUrl',$this->forward);
    		//$this->error(L('logined'));
    		redirect(U('User/Login/index?l='.LANG_NAME));
