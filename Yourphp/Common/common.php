	...
	if($name=='Lang'){
		...
		if(APP_LANG){
			...
			foreach((array)$lang as $key=>$r){
				...
				if($langid){
					$lang = $key;
					$list = $Model->where('lang='.$langid)->order('listorder = 0,listorder asc,id desc')->select ();
					$pkid = $Model->getPk ();
					$data = array ();
					foreach ( $list as $key => $val ) {
						$data [$val [$pkid]] = $val;
			...
			$list = $Model->order('listorder = 0,listorder asc,id desc')->select ();
			$pkid = $Model->getPk ();
			$data = array ();
			foreach ( $list as $key => $val ) {
				$data [$val [$pkid]] = $val;
					...
					if(isset($postdata['auto_thumb']) && $postdata['thumb'] == '' && isset($postdata['content'])) {
						...
						if(preg_match_all("/(src)=([\"|']?)([^ \"'>]+\.(gif|jpg|jpeg|bmp|png))\\2/i", $content, $matches)) {
							$postdata['thumb'] = $matches[$auto_thumb_no][0];
                                    ...
                                    $postdata[$key] =  safe_replace(strip_tags($postdata[$key]));
                                    //英文title可以输入' 
                                    $postdata[$key] = addslashes(strip_tags($postdata[$key]));
				}
		}
		return $postdata;
}
...
function safe_replace($string) {
	...
	//$string = str_replace(array('\\',';','\'','%2527','%27','%20','&', '"', '<', '>'), array('','','','','','','&amp;', '&quot;', '&lt;', '&gt;'), $string);
        $string = str_replace(array('%2527','%27','%20','&', '"', '<', '>'), array('','','','&amp;', '&quot;', '&lt;', '&gt;'), $string);
	$string = str_replace(array('user()', 'sleep(', 'length(', 'insert', 'select'), array('','','','',''), $string);
	$string=nl2br($string); 
	return $string;
}	
...
function curl_get($url, $timeout = 20, $port = 80)
{
    	$ch = curl_init();
...
function sendemail($tomail,$toname,$subject,$body,$config){
	import("@.ORG.PHPMailer");
	$mailer = new PHPMailer();

	$mailer -> Host = $config['mail_server'];
	$mailer -> Port = $config['mail_port'];
	// 用户名
    $mailer -> Username = $config['mail_user'];
    // 密码
    $mailer -> Password = $config['mail_password'];
    $mailer -> From = $config['mail_from'];
...
function send_smtp($receiver, $subject, $content,$config)
    {
        import("@.ORG.cls_smtp");
        ...
        $headers[] = 'From: "' . '=?' . $charset . '?B?' . base64_encode($config['site_name']) . '?='.'" <'.$config['mail_from'].'>';
        ...
        /* 获得邮件服务器的参数设置 */
        $params['host'] = $config['mail_server'];
        $params['port'] = $config['mail_port'];
        $params['user'] = $config['mail_from'];
        $params['pass'] = $config['mail_password'];
        $send_params['recipients'] = $receiver;
        $send_params['headers']    = $headers;
        $send_params['from']       = $config['mail_from'];
