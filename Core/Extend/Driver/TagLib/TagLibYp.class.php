...
class TagLibYp extends TagLib
{
	protected $tags   =  array(
		...
		'next' => array('attr'=>'blank,msg','close'=>0),
            'newslist'=>array('attr'=>'name,field,limit,order,catid,thumb,posid,where,sql,key,page,mod,id,ids,status','level'=>3),
            'newslist2'=>array('attr'=>'name,field,limit,order,catid,thumb,posid,where,sql,key,page,mod,id,ids,status','level'=>3),
    );
	...
	public function _nav($attr,$content) {
		...
		if($catid && !is_numeric($catid)){
 		...
 		if(is_array($category_arr)){
			...
			import ( '@.ORG.Tree' );
                        $parsestr = $this->get_menu($array);
			//$tree = new Tree ($array);var_Dump($cata_arr_res);exit;
			//$parsestr = $tree->get_nav($catid,$level,$id,$class,$homefont,FALSE,'',$enhomefont,$lang);
                        //var_Dump($parsestr);exit;
		}unset($category_arr);
		return  $parsestr;
	}
        
        public static function get_menu($cate_arr){
            $cata_arr_res = array();
            if($cate_arr)foreach($cate_arr as $v){
                $cata_arr_res[$v['parentid']][] = $v;
            }
            $menu = '';
            if(isset($cata_arr_res[0]) && !empty($cata_arr_res[0])){
                $count = count($cata_arr_res[0]);
                $i = 0;
                foreach($cata_arr_res[0] as $v){
                    $i ++;
//                     if($v['id'] == 31){
//                     	$purl = U('Activity/index');
//                     }else{
//                     	$purl = $v['url'];
//                     }
                    $purl = $v['url'];
                    if($v['id'] == 30 || $v['id'] == 48){
                        $purl = $v['url'].'&year='.date('Y');
                    }else{
                    	$purl = $v['url'];
                    }
//                    <li class="nav_item">
...
//                    </li>
                    ...
                    if(isset($cata_arr_res[$v['id']]) && !empty($cata_arr_res[$v['id']])){
                        ...
                        array_multisort($c,SORT_ASC,$cc);
                        array_multisort($d,SORT_ASC,$dd);
                        $res = array_merge($cc,$dd);
                        foreach($res as $sv){
                            $menu .= '<a href="'.$sv['url'].'" >'.$sv['catname'].'</a>';
                        }
                        $menu .= '</div>';
                    }
                    $menu .= "</li>";
	...
	public function _list($attr,$content) {
                        global $webInfo;
                        $webid = isset($webInfo['self']['id']) ? $webInfo['self']['id'] : 1;
			$tag    = $this->parseXmlAttr($attr,'list');
			$id = !empty($tag['id'])?$tag['id']:'r';  //定义数据查询的结果存放变量
			$key    = !empty($tag['key'])?$tag['key']:'i';
			$page   = !empty($tag['page'])? '1' : '0';
			$mod    = isset($tag['mod'])?$tag['mod']:'2';
                        if(LANG_ID == 2 && $tag['catid'] == 30){
                            $tag['catid'] = 48;
                        }
        ...
        public function _newslist($attr,$content) {
                        ...
                        $webid = isset($webInfo['self']['id']) ? $webInfo['self']['id'] : 1;
                        $webids = array();
                        if($webInfo['list'])foreach($webInfo['list'] as $v){
                            if($v['id'] != $webid){
                                $webids[] = $v['id'];
                                if(count($webids) == 2) break;
                            }
                        }
                        ...
                        $status = isset($tag['status'])? intval($tag['status']) : '1';
                        $ids =  isset($tag['ids'])?$tag['ids']:'';
                        $catid = isset($tag['catid'])?$tag['catid']:30;
                        if(APP_LANG){
                                $lang=$this->tpl->get('langid');
                                $where .= ' and lang='.$lang;
                        }
                        $where .= ' and website='.$webids[0];
        ...
        public function _newslist2($attr,$content) {
                        global $webInfo;
                        $webid = isset($webInfo['self']['id']) ? $webInfo['self']['id'] : 1;
                        $webids = array();
                        if($webInfo['list'])foreach($webInfo['list'] as $v){
